...
@register("astrbot_plugin_sadstory", "Towqs", "伤感故事插件 - 以合并转发形式在群聊中展示伤感故事", "0.5.0")
...
"""
    ...
    async def llm_add_writing_style(
        ...
        """
        向伤感故事插件的数据库中添加一个写作风格。写作风格是一段 prompt 指令，用于指导 AI 如何生成伤感故事。
    ...
    async def llm_add_story_template(
        ...
        """
        向伤感故事插件的数据库中添加一个故事模板。故事模板是一段完整的伤感故事范文，AI 生成新故事时会参考它的叙事风格和情感表达。
        故事模板应该是一段高质量的伤感故事，包含主角讲述和围观网友互动，模拟QQ群聊天的形式。
        Args:
    ...
    async def ai_add_template(self, event: AiocqhttpMessageEvent):
        ...
        system_prompt = (
            "你是伤感故事插件的故事模板生成助手。用户会给你一段故事描述，"
            "你需要根据描述创作一篇完整的伤感故事范文，然后调用 llm_add_story_template 工具写入数据库。\n\n"
            "故事模板的规范：\n"
