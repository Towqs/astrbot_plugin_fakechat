...
@register("astrbot_plugin_sadstory", "Towqs", "伪装聊天插件 - 以合并转发形式在群聊中展示伪装聊天", "0.5.2")
class SadStoryPlugin(Star):
    ...
    async def _get_active_prompt_style(self) -> str:
...
"""
        ...
        try:
            ...
            for item in story_data:
                ...
                if not speaker or not content:
                    continue
                user_info = role_map.get(speaker, protagonist)  # fallback 到主角而非 random.choice
                messages.append({
    ...
    async def sadstory(self, event: AiocqhttpMessageEvent):
        """发送一段伪装聊天（合并转发形式）。用法：/sadstory [主题]，仅管理员可用"""
        self._reload_config()
        await self._import_webui_data()
    ...
    async def show_styles(self, event: AiocqhttpMessageEvent):
        """查看写作风格列表。用法：/sadstory_style，仅管理员可用"""
        styles = await self.db.get_styles()
        lines = []
        if styles:
            en = sum(1 for _, _, e, _ in styles if e)
    ...
    async def llm_add_writing_style(
        self, event: AiocqhttpMessageEvent, style_name: str, style_content: str
    ):
        """
        向伪装聊天插件的数据库中添加一个写作风格。写作风格是一段 prompt 指令，用于指导 AI 如何生成伪装聊天。
        写作风格内容必须包含以下占位变量：{protagonist}（主角名）、{bystanders}（围观网友名）、{min_msg}（最少消息数）、{max_msg}（最多消息数）。
        可选变量：{theme_line}（主题行）、{reference_section}（参考故事段）、{emoji_instruction}（表情说明）。
        末尾必须要求输出 JSON 数组格式：[{{"speaker": "角色名", "content": "台词内容"}}, ...]。
        如果用户提供的内容不符合规范，请先修改使其符合后再调用此工具写入。
        Args:
            style_name(string): 写作风格的名称，简短描述该风格特点，如"温柔治愈风"、"黑色幽默风"
            style_content(string): 完整的写作指令内容（prompt），必须包含 {protagonist} {bystanders} {min_msg} {max_msg} 占位符，末尾需要求 JSON 数组输出格式
        """
        # 校验必需占位符
        ...
        if missing:
            yield event.plain_result(
                f"写入失败：风格内容缺少必需变量 {', '.join(missing)}\n"
                "请补充后重试。"
            )
            return
        ...
        yield event.plain_result(
            f"写作风格「{style_name}」已写入数据库（ID:{sid}，{len(style_content)}字）"
        )

    @filter.llm_tool()  # type: ignore
    async def llm_add_story_template(
        self, event: AiocqhttpMessageEvent, tpl_name: str, tpl_content: str
    ):
        """
        向伪装聊天插件的数据库中添加一个故事模板。故事模板是一段完整的伪装聊天范文，AI 生成新内容时会参考它的叙事风格和情感表达。
        故事模板应该是一段高质量的伪装聊天，包含主角讲述和围观网友互动，模拟QQ群聊天的形式。
        Args:
            tpl_name(string): 模板名称，简短描述故事类型，如"校园暗恋"、"异地分手"、"错过的人"
            tpl_content(string): 完整的故事范文内容，要求情感真挚、叙事完整、有起承转合，结尾有余韵
        """
        if len(tpl_content) < 50:
            yield event.plain_result("写入失败：故事模板内容太短（至少50字），请补充更完整的故事")
            return

        tpl_id = await self.db.add_template(tpl_name, tpl_content)
        yield event.plain_result(
            f"故事模板「{tpl_name}」已写入数据库（ID:{tpl_id}，{len(tpl_content)}字）"
        )
    ...
    async def ai_add_style(self, event: AiocqhttpMessageEvent):
        ...
        system_prompt = (
            "你是伪装聊天插件的写作风格生成助手。用户会给你一段风格描述，"
            "你需要根据描述生成一个完整的写作风格 prompt，然后调用 llm_add_writing_style 工具写入数据库。\n\n"
            "写作风格 prompt 的规范：\n"
        ...
        func_tools_mgr = self.context.get_registered_star_tools_manager()
        try:
            await self.context.tool_loop_agent(
                event=event,
                system_prompt=system_prompt,
                user_prompt=f"请根据以下描述生成写作风格并写入：{desc}",
                func_tools_mgr=func_tools_mgr,
            )
        except Exception as e:
    ...
    async def ai_add_template(self, event: AiocqhttpMessageEvent):
        ...
        system_prompt = (
            "你是伪装聊天插件的故事模板生成助手。用户会给你一段故事描述，"
            "你需要根据描述创作一篇完整的伪装聊天范文，然后调用 llm_add_story_template 工具写入数据库。\n\n"
            "故事模板的规范：\n"
        ...
        try:
            await self.context.tool_loop_agent(
                event=event,
                system_prompt=system_prompt,
                user_prompt=f"请根据以下描述创作故事模板并写入：{desc}",
                func_tools_mgr=func_tools_mgr,
            )
        except Exception as e:
