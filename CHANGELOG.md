...
## v0.9.3
- **重磅功能：永久人设夺取系统**：引入数据库级的「灵魂容器」记忆体系。
  - 新增 `/sadstory_capture @张三 [条数]` 指令（管理员专属）。系统会在后台分页追溯群历史消息，精准过滤出只属于该人的发言，交给大模型生成一份「人物画像卡」，并永久固化进 SQLite 数据库的 `user_personas` 表中。
  - 从此以后，只要此人被设定为小剧场主角，大模型都会在底层深处强行加载并附体他的专属人设卡。无需每次生成再重新抓取聊天记录，彻底消灭延迟与 Token 开销。
  - 新增 `/sadstory_listp` 查看灵魂图鉴；`/sadstory_delp` 将某个灵魂放归自由。
- **权限系统全面纠正**：废除所有模块中的 `@filter.permission_type(PermissionType.ADMIN)` 装饰器，统一改用插件内部 `_is_admin` 手动校验。彻底解决了指令被框架静默拦截后漏给 LLM Agent 导致乱禁言的历史级 Bug。
- **人设语气优化**：人格测写提示词从"极度毒舌的心理侦探"调整为"幽默风趣的群友画像师"，生成内容从人身攻击风转为善意调侃风。
- **兼容性**：新增适配 AstrBot 框架转码的 `[At:xxx]` 艾特文本格式解析。
- **翻页修复**：修正群历史消息分页回溯的 `message_seq` 计算逻辑，不再触发"消息不存在"报错。
