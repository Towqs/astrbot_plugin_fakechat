...
- 🖼️ **支持贴纸表情包联动**：大模型可在情绪激动时发送你自定义的梗图贴纸
- 🎭 支持从指定群读取真实用户作为角色（昵称+头像）
- 👥 也支持手动配置虚拟角色
- 📝 支持导入故事模板作为 LLM 参考素材，生成风格更贴近真实
- 🧠 **永久人设夺取系统**：抓取群友真实聊天记录，由大模型总结人物画像并存入数据库，生成时自动附体
- 🤖 支持自定义 LLM 模型
- 🔒 仅管理员可触发，支持冷却时间
...
为了让伪装聊天更具网感，插件支持让大模型自动在发送消息时穿插你本地的表情包（贴纸图片）。

**如何使用**：
1. **前置插件**：本插件自身不管理图片列表，你**必须**在 AstrBot 中搜寻安装并启用 `astrbot_plugin_meme_manager_lite` 插件。
2. **存放贴纸**：依照 `meme_manager_lite` 的使用说明，将你想让 AI 使用的表情包分类放入该插件的数据目录，配好标签名称。
3. **开启开关**：最后，在 `sadstory` 的 WebUI 配置页面中，开启「启用贴纸功能」。此时 AI 就能看到你的全部表情包并在生草时刻自动发图了！
...
| 命令 | 说明 |
|------|------|
| `/sadstory_nest @外层 @A` | 单主角嵌套模式，外层用户发送转发消息 |
| `/sadstory_nest @外层 @A @B` | 双主角嵌套模式 |
| `/sadstory_nest @外层 @A @B 主题` | 指定主题的嵌套模式 |

**嵌套模式说明**：
- 第一个 `@` 是外层发送者（转发消息的人）
- 后续 `@` 是内层聊天的主角
- 外层发送者固定，不会变成其他人
- 无主题时内层聊天跟主故事相关，有主题时内层随机
...
| 命令 | 说明 | 权限 |
|------|------|------|
| `/sadstory_capture @目标 [条数]` | 抓取目标用户的群聊历史发言，由 AI 生成人物画像并存入数据库 | 管理员 |
| `/sadstory_listp` | 查看当前数据库中所有已捕获的人设档案 | 管理员 |
| `/sadstory_delp @目标` | 删除某人的人设档案，恢复默认生成 | 管理员 |

> 🧠 **人设系统说明**：使用 `/sadstory_capture` 后，系统会自动分页翻找群历史消息，过滤出只属于该用户的发言（默认 50 条，可指定 10-100），交给大模型生成一份“人物画像卡”并永久保存到数据库。此后每次用 `/sadstory` 生成小剧场时，只要该用户被选为主角，大模型就会自动按照他的真实说话风格来生成台词，无需重复抓取。
